...
export class SurveyEventsOverviewPageComponent implements OnInit, OnDestroy {
    ...
    getSurveyEvents() {
        ...
        this.surveyEventsService
            .getAllSurveyEvents(page, size, filter)
            .pipe(take(1))
            .subscribe((value) => {
                this.pager = value;
    ...
    getSpecies(searchQuery: string, id?: number) {
        this.taxaService
            .getTaxa(searchQuery, id)
            .subscribe(taxa =>
                this.species = taxa
                    .map(taxon => ({
    ...
    private setTags() {
        ...
        if (rawValue.measuringPointNumber) {
            tags.push(getTag('surveyEvent.measuringPointNumber', rawValue.measuringPointNumber, this.removeTagCallback('measuringPointNumber')));
        }
